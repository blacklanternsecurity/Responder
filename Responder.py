...
parser.add_option('--smb2support',        action="store_true",      help="Enable SMB2 Support (experimental)", dest="SMB2Support", default=False)
...
def main():
	try:
		...
		if settings.Config.SMB_On_Off:
			if settings.Config.SMB2Support:
				from servers.SMB import SMB2
				threads.append(Thread(target=serve_thread_tcp, args=(settings.Config.Bind_To, 445, SMB2,)))
				threads.append(Thread(target=serve_thread_tcp, args=(settings.Config.Bind_To, 139, SMB2,)))
